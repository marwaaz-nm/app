  ...
  try {
    ...
    if (query.length >= 2) {
      ...
      if (hasMenu(viewer, '/records')) {
        searches.push(viewer.admin.from('surveys').select('id, serial_no, owner_name, neighborhood, status').ilike('owner_name', pattern).limit(5));
        searches.push(viewer.admin.from('surveys').select('id, serial_no, owner_name, neighborhood, status').ilike('neighborhood', pattern).limit(5));
      }
    ...
    const [pendingResult, rejectedResult, referenceResult] = await Promise.all([
      hasMenu(viewer, '/records')
        ? viewer.admin.from('surveys').select('id, serial_no, owner_name, updated_at').eq('status', 'Pending Review').order('updated_at', { ascending: false }).limit(8)
        : Promise.resolve({ data: [], error: null }),
      hasMenu(viewer, '/records')
        ? viewer.admin.from('surveys').select('id, serial_no, owner_name, rejection_reason, updated_at').eq('status', 'Rejected').order('updated_at', { ascending: false }).limit(5)
        : Promise.resolve({ data: [], error: null }),
