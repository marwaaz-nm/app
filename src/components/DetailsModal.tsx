...
export default function DetailsModal({ record, onClose }: DetailsModalProps) {
  ...
  const parsePolygonCoords = (
    polyString: string | undefined
  ): [number, number][] => {
    if (!polyString || polyString === "N/A" || polyString.trim() === "")
      return [];
    try {
      return polyString
        .split(";")
    ...
    return directions
      .map((dir) => {
        ...
        if (parts.length === 2) {
          ...
          const deriska = subParts[1]
            ? subParts[1].replace(")", "").trim()
            : "-";
  ...
  const addSketchDimension = (
    ...
    L.polyline([p1, p2], { color: "#3b82f6", weight: 1.5, opacity: 0.8 }).addTo(
      map
    );
    ...
    L.polyline([end, p2], { color: "#3b82f6", weight: 1, opacity: 0.5 }).addTo(
      map
    );
  ...
  const computeAutoDirectionPosition = (
    start: L.LatLng,
    end: L.LatLng,
    allCoords: L.LatLng[]
  ): L.LatLng => {
    ...
    for (const direction of directions) {
      ...
      if (manual) {
        renderDirectionLabel(
          direction,
          L.latLng(manual.lat, manual.lng),
          map,
          boundaryInfo,
          manual.rotation ?? 0,
          sharedDirectionFontSize
        );
        continue;
      }
      for (const idx of mainBoundaryIndices) {
        ...
        if (
          getDirectionFromCenter(center.lat, center.lng, mid.lat, mid.lng) ===
          direction
        ) {
          renderDirectionLabel(
            direction,
            computeAutoDirectionPosition(start, end, latlngs),
            map,
            boundaryInfo,
            0,
            sharedDirectionFontSize
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(() => {
      ...
      const vertexCenter = latlngs.reduce(
        (acc, c) => ({
          lat: acc.lat + c.lat / latlngs.length,
          lng: acc.lng + c.lng / latlngs.length,
        }),
        { lat: 0, lng: 0 }
      );
      ...
      const manualPositions = parseDirectionPositions(
        record.boundary_label_positions
      );
      const savedSketchParts = (record.sketch_dimensions || "")
        .split("|")
        .map((part) => part.trim());
      const savedSketchDimensions = savedSketchParts
        .slice(1)
        .map((part, index) =>
      ...
      const segmentDistances = latlngs.map((startPt, idx) => {
        ...
        return {
          index: idx,
          dist: L.latLng(startPt).distanceTo(L.latLng(endPt)),
        };
      });
      ...
      if (satelliteMapContainerRef.current) {
        ...
        satelliteMapContainerRef.current.addEventListener(
          "wheel",
          handleSatWheel,
          { passive: false }
        );
      ...
      if (sketchMapContainerRef.current) {
        ...
        for (let i = 0; i < latlngs.length; i++) {
          ...
          const direction = getDirectionFromCenter(
            vertexCenter.lat,
            vertexCenter.lng,
            midpoint.lat,
            midpoint.lng
          );
          const savedBoundaryValue = mainBoundaryIndices.has(i)
            ? boundaryInfo[direction]?.val
            : undefined;
          addSketchDimension(
            start,
            end,
            skMap,
            latlngs,
            savedSketchDimensions[i] || savedBoundaryValue
          );
        }
        addBoundaryDirectionLabels(
          skMap,
          latlngs,
          mainBoundaryIndices,
          vertexCenter,
        ...
        const matchArea =
          record.sketch_dimensions?.match(/Area:\s*([^\s|]+)/i) ||
          record.sketch_dimensions?.match(/Area\s*([^\s|]+)/i);
        ...
        sketchMapContainerRef.current.addEventListener("wheel", handleSkWheel, {
          passive: false,
        });
      }
    }, 450);
    ...
    return () => {
      clearTimeout(timer);
      if (satelliteMapContainerRef.current) {
        satelliteMapContainerRef.current.removeEventListener(
          "wheel",
          handleSatWheel
        );
      }
      if (sketchMapContainerRef.current) {
        sketchMapContainerRef.current.removeEventListener(
          "wheel",
          handleSkWheel
        );
      }
  ...
  const handlePrintPDF = async () => {
    ...
    try {
      ...
      showAlert(
        "Sug fadlan...",
        "PDF-ka ayaa la diyaarinayaa, fadlan sug...",
        "info"
      );
      ...
      if (sketchMapContainerRef.current) {
        ...
        if (pdfSketchCoords.length >= 3) {
          ...
          Object.values(
            parseDirectionPositions(record.boundary_label_positions)
          ).forEach((position) => {
            if (position) pdfSketchBounds.extend([position.lat, position.lng]);
          });
          sketchMap?.fitBounds(pdfSketchBounds, {
            animate: false,
            padding: [20, 20],
          });
        }
        ...
        const areaMarker = sketchContainer.querySelector(
          ".sketch-area-label"
        ) as HTMLElement | null;
        ...
        try {
          const sketchCanvas = await html2canvas(
            sketchMapContainerRef.current,
            {
              useCORS: true,
              allowTaint: true,
              scale: 2,
          ...
          sketchMap?.invalidateSize({ animate: false });
          if (pdfSketchCoords.length >= 3) {
            sketchMap?.fitBounds(L.polygon(pdfSketchCoords).getBounds(), {
              animate: false,
              padding: [60, 60],
            });
          }
      ...
      const gpsParts = record.gps_location
        ? record.gps_location.split(",")
        : [];
      const latVal = gpsParts[0] ? gpsParts[0].trim() : "N/A";
      const lngVal = gpsParts[1] ? gpsParts[1].trim() : "N/A";
      const areaClean = record.sketch_area
        ? record.sketch_area.replace(" m²", "").replace("m²", "").trim()
        : "N/A";
      const issueDate = record.created_at
      ...
      const footerHTML = (page: number, title: string) => `
        <div style="border-top:1px solid #cbd5e1;padding-top:9px;margin-top:auto;display:grid;grid-template-columns:1fr auto 1fr;align-items:center;font-size:9px;color:#475569;line-height:1.45;font-family:Arial,sans-serif;position:relative;z-index:1;">
          <div style="text-align:left;">${settings.contact_phone || ""}<br/>${
        settings.contact_email || ""
          ...
          }<br/>Generated by Marwaazpn App</div>
        </div>
      `;
        ...
        <div class="survey-pdf-page" style="width:750px;height:1060px;padding:32px 38px 28px;box-sizing:border-box;display:flex;flex-direction:column;justify-content:space-between;font-family:Arial,sans-serif;background:#ffffff;color:#000000;position:relative;overflow:hidden;">
          ...
          <div style="position:relative;z-index:1;">
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;font-size:11px;font-weight:800;margin:14px 0 12px;color:#17324d;">
              <span style="background:#eaf7ee;border:1px solid #b8e1c2;border-radius:999px;padding:6px 12px;">Survey No: ${
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:8px;font-size:11px;color:#17324d;margin-bottom:18px;">
              <div style="grid-column:1 / -1;border-left:4px solid #168b35;background:#f5faf6;padding:9px 12px;border-radius:4px;"><span style="font-size:8px;text-transform:uppercase;color:#64748b;font-weight:800;">Milkiilaha / Owner</span><br/><strong style="font-size:13px;">${
                record.owner_name
              }</strong></div>
              <div style="border:1px solid #dce7ef;padding:8px 10px;border-radius:6px;background:#ffffff;"><span style="font-size:8px;color:#64748b;font-weight:800;">GOOBTA</span><br/><strong>${
                record.neighborhood
              }${record.branch ? " - " + record.branch : ""}</strong></div>
              <div style="border:1px solid #dce7ef;padding:8px 10px;border-radius:6px;background:#ffffff;"><span style="font-size:8px;color:#64748b;font-weight:800;">NOOCA DHULKA</span><br/><strong>${
                record.land_type || "-"
              }</strong></div>
              ...
              <table style="width:100%;border-collapse:collapse;border:1px solid #bfd2df;font-size:11px;">
                ...
                <tbody>
                  <tr style="border-bottom:1px solid #000000;">
                    ...
                    <td style="padding:8px 12px;border:1px solid #000000;text-align:left;vertical-align:middle;">${cleanVal(
                      record.boundary_w_val
                    )}</td>
                  </tr>
                  <tr style="border-bottom:1px solid #000000;">
            ...
            <div style="margin-bottom:20px;">
              ...
              <table style="width:100%;border-collapse:collapse;border:1px solid #bfd2df;font-size:11px;">
                ...
                <tbody>
                  <tr style="border-bottom:1px solid #000000;">
                    <td style="padding:8px 12px;border:1px solid #000000;text-align:left;vertical-align:middle;">North</td>
                    <td style="padding:8px 12px;border:1px solid #000000;text-align:left;vertical-align:middle;">${
                      record.boundary_w_neighbor || "-"
                    }</td>
                  </tr>
                  <tr style="border-bottom:1px solid #000000;">
                    <td style="padding:8px 12px;border:1px solid #000000;text-align:left;vertical-align:middle;">East</td>
                    <td style="padding:8px 12px;border:1px solid #000000;text-align:left;vertical-align:middle;">${
                      record.boundary_b_neighbor || "-"
                    }</td>
                  </tr>
                  <tr style="border-bottom:1px solid #000000;">
                    <td style="padding:8px 12px;border:1px solid #000000;text-align:left;vertical-align:middle;">West</td>
                    <td style="padding:8px 12px;border:1px solid #000000;text-align:left;vertical-align:middle;">${
                      record.boundary_g_neighbor || "-"
                    }</td>
                  </tr>
                  <tr style="border-bottom:1px solid #000000;">
                    <td style="padding:8px 12px;border:1px solid #000000;text-align:left;vertical-align:middle;">South</td>
                    <td style="padding:8px 12px;border:1px solid #000000;text-align:left;vertical-align:middle;">${
                      record.boundary_k_neighbor || "-"
                    }</td>
                  </tr>
                </tbody>
              </table>
        ...
        <div class="survey-pdf-page" style="width:750px;height:1060px;padding:32px 38px 28px;box-sizing:border-box;display:flex;flex-direction:column;justify-content:space-between;font-family:Arial,sans-serif;background:#ffffff;color:#000000;position:relative;overflow:hidden;">
          ...
          <div style="position:relative;z-index:1;">
            ...
            <div style="display:flex;justify-content:space-between;align-items:center;background:#f5f8fa;border:1px solid #dbe6ed;border-radius:6px;padding:8px 11px;margin:12px 0 12px;font-size:9px;color:#64748b;">
              <span>Survey: <strong style="color:#17324d;">${
                record.survey_no || record.serial_no
              }</strong></span>
            ...
            <div style="display:grid;grid-template-columns:1fr;gap:12px;align-items:stretch;">
              <div style="border:1px solid #b9cbd7;border-left:5px solid #2f86c7;border-radius:7px;padding:10px;background:#ffffff;box-sizing:border-box;height:292px;display:grid;grid-template-columns:160px 1fr;gap:12px;align-items:stretch;">
                ...
                <div style="min-width:0;display:flex;align-items:center;justify-content:center;overflow:hidden;background:#ffffff;">
                  ${
                    sketchImage
                      ? `<img src="${sketchImage}" style="width:100%;height:268px;object-fit:contain;display:block;background:#ffffff;" />`
                      : `<div style="color:#64748b;font-size:11px;">Sketch image not available</div>`
                  }
                </div>
              </div>
              <div style="border:1px solid #b9cbd7;border-left:5px solid #168b35;border-radius:7px;padding:10px;background:#ffffff;box-sizing:border-box;height:292px;display:grid;grid-template-columns:160px 1fr;gap:12px;align-items:stretch;">
                ...
                <div style="min-width:0;display:flex;align-items:center;justify-content:center;overflow:hidden;background:#e2e8f0;">
                  ${
                    satImage
                      ? `<img src="${satImage}" style="width:100%;height:268px;object-fit:cover;display:block;background:#e2e8f0;" />`
                      : `<div style="color:#64748b;font-size:11px;">Satellite map image not available</div>`
                  }
                </div>
              </div>
            </div>
      ...
      const classicHeader = (includeSomaliTitle: boolean) => `
        ...
        ${
          includeSomaliTitle
            ? `<div style="text-align:center;font-weight:800;line-height:1.15;margin:1px 0 8px;"><div style="font-size:14px;color:#0865ed;">Jamhuuriyadda Federaalka Soomaaliya</div><div style="font-size:14px;color:#c40000;">Nootaayo Marwaaz</div></div>`
            : ""
        }
      ...
      const sideRows = [
        [
          "Waqooyi / North",
          cleanVal(record.boundary_w_val),
          record.boundary_w_neighbor || "-",
        ],
        [
          "Bari / East",
          cleanVal(record.boundary_b_val),
          record.boundary_b_neighbor || "-",
        ],
        [
          "Galbeed / West",
          cleanVal(record.boundary_g_val),
          record.boundary_g_neighbor || "-",
        ],
        [
          "Koonfur / South",
          cleanVal(record.boundary_k_val),
          record.boundary_k_neighbor || "-",
        ],
      ]
        .map(
          ([side, length, neighbour]) =>
            `<tr><td style="font-weight:700;">${side}</td><td>${length}m</td><td>${neighbour}</td></tr>`
        )
        .join("");
      ...
      printContainer.innerHTML = `
        <div class="survey-pdf-page" style="width:750px;height:1060px;padding:24px 42px 20px;box-sizing:border-box;display:flex;flex-direction:column;font:12px/1.15 Arial,sans-serif;background:#fff;color:#000;position:relative;overflow:hidden;">
          ...
          <div style="position:relative;z-index:1;">${classicHeader(true)}
            <div style="display:flex;justify-content:space-between;font-size:12px;font-weight:700;padding:0 5px 6px;"><span>Sumad No: ${
              record.survey_no || record.serial_no
            }</span><span>Taariikh: ${issueDate}</span></div>
            ...
            <table class="classic-survey-table" style="width:100%;border-collapse:collapse;table-layout:fixed;font-size:14px;margin-bottom:11px;">
              ...
              <tr><td>${record.owner_name}</td><td>${record.neighborhood}${
        record.branch ? " - " + record.branch : ""
            ...
            <div style="height:400px;border:1px solid #1683df;background:#fff;display:flex;align-items:center;justify-content:center;overflow:hidden;">
              ${
                sketchImage
                  ? `<img src="${sketchImage}" style="width:100%;height:100%;object-fit:contain;display:block;" />`
                  : ""
              }
            </div>
          </div>
        ...
        <div class="survey-pdf-page" style="width:750px;height:1060px;padding:20px 46px 20px;box-sizing:border-box;display:flex;flex-direction:column;font:14px/1.2 Arial,sans-serif;background:#fff;color:#000;overflow:hidden;">
          ...
          <div style="height:695px;border:1px solid #1683df;background:#e2e8f0;overflow:hidden;">
            ${
              satImage
                ? `<img src="${satImage}" style="width:100%;height:100%;display:block;" />`
                : ""
            }
          </div>
      ...
      const pages = Array.from(
        printContainer.querySelectorAll(".survey-pdf-page")
      ) as HTMLElement[];
      ...
      const fileName = `Survey_Report_SN_${
        record.serial_no
      }_${record.owner_name.replace(/\s+/g, "_")}.pdf`;
      ...
      if (isMobileDevice) {
        ...
        if (
          ...
          showAlert(
            "Guul",
            "PDF-ka waa la diyaariyey. Ka dooro Save ama app-ka aad rabto.",
            "success"
          );
        } else {
  ...
  return createPortal(
    <div className="fixed inset-0 z-[1300] flex items-center justify-center md:p-4 bg-slate-950/60 backdrop-blur-sm overflow-y-auto">
      <div className="relative w-full h-full md:h-auto md:max-w-6xl bg-white border-0 md:border md:border-slate-100 md:rounded-3xl overflow-hidden shadow-2xl flex flex-col my-0 md:my-8 animate-in fade-in md:zoom-in-95 duration-200 text-slate-800">
          ...
          <div className="flex items-center gap-3 min-w-0">
            ...
            <div className="min-w-0">
              <h3 className="font-extrabold text-sm text-slate-800 truncate">
                Warbixinta Sahanka Dhulka
              </h3>
              <p className="text-xs text-slate-500 font-semibold truncate">
                Record details and spatial parameters
              </p>
            </div>
          </div>
        ...
        {/* Modal Body */}
        <div
          className="p-4 md:p-5 overflow-y-auto flex-1 md:max-h-[calc(90vh-80px)]"
          id="survey-report-print"
        >
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              ...
              <div className="flex items-center gap-4 p-4 border border-slate-200 rounded-2xl bg-white shadow-[0_2px_8px_rgba(0,0,0,0.02)]">
                ...
                <div>
                  <span className="block text-xs font-black uppercase tracking-wider text-slate-400 mb-0.5">
                    MILKILAHA DHULKA
                  </span>
              ...
              {record.vicinity && (
                <div className="p-3.5 border border-slate-200 rounded-2xl bg-white shadow-[0_2px_8px_rgba(0,0,0,0.02)]">
                  <span className="block text-xs font-black uppercase tracking-wider text-slate-400 mb-1">
                    NAWAAXIGA (Vicinity)
                  </span>
                  <div className="text-sm font-bold text-slate-800">
                    {record.vicinity}
                    ...
                    <tbody className="divide-y divide-slate-100">
                      {boundaries.length > 0 ? (
                        ...
                        <tr>
                          <td
                            colSpan={3}
                            className="px-4 py-4 text-center text-slate-400 font-semibold italic"
                          >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div className="flex items-center gap-3 p-3.5 border border-slate-200 rounded-2xl bg-white shadow-[0_2px_8px_rgba(0,0,0,0.02)]">
                  ...
                  <div>
                    ...
                    <code className="text-sm font-bold text-teal-600 block select-all truncate">
                      {record.gps_location || "N/A"}
                    </code>
                  </div>
                </div>
              </div>
              ...
              {record.built_details && (
                <div className="p-3 border border-rose-100 rounded-2xl bg-rose-50/50">
                  <span className="block text-xs font-extrabold uppercase tracking-wider text-rose-600 mb-1">
                    WAXA KU DHISAN
                  </span>
                  <div className="text-sm font-bold text-slate-800">
                    {record.built_details}
            ...
            <div className="space-y-4">
              ...
              <div
                className={
                  isSatFullscreen
                    ? "fixed inset-4 md:inset-10 z-50 bg-white shadow-2xl flex flex-col rounded-3xl border border-slate-200 overflow-hidden animate-in fade-in zoom-in-95 duration-200"
              ...
              >
                ...
                <div
                  ...
                  style={
                    isSketchFullscreen
                      ? { height: "100%", flex: 1, backgroundColor: "#ffffff" }
                      : { height: "320px", backgroundColor: "#ffffff" }
                  }
            ...
            {linkedRefsLoading ? (
              ...
              <div className="flex flex-col items-center gap-2 py-10 text-center">
                ...
                <p className="text-xs font-semibold text-slate-400">
                  Weli ma jiro ref number dhulkan lagu isticmaalay.
                </p>
              </div>
            ) : (
              <div className="space-y-2.5">
                {linkedRefs.map((ref) => (
                  <div
                    key={ref.id}
                    className="rounded-2xl border border-slate-200 bg-white p-3.5 shadow-[0_2px_8px_rgba(0,0,0,0.02)]"
                  >
                    ...
                    <p className="mt-1 text-[10px] font-medium text-slate-400">
                      {ref.issue_date
                        ? new Date(ref.issue_date).toLocaleDateString("so-SO")
                        : ref.created_at
                        ? new Date(ref.created_at).toLocaleDateString("so-SO")
                        : "-"}
                    </p>
                  </div>
                ))}
