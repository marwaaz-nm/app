...
export default function Sidebar() {
  ...
  const primaryMobileNav = PRIMARY_MOBILE_HREFS.map((href) =>
    permittedNavigation.find((item) => item.href === href)
  ).filter((item): item is NavItem => Boolean(item));
  const moreMobileNav = permittedNavigation.filter(
    (item) => !PRIMARY_MOBILE_HREFS.includes(item.href)
  );
  const moreActive = moreMobileNav.some((item) => isActive(item.href));
  const moreUnreadCount = moreMobileNav.reduce(
    (total, item) => total + unreadCountFor(item.href),
    0
  );
      ...
      <nav
        ...
        style={{
          gridTemplateColumns: `repeat(${
            primaryMobileNav.length + (moreMobileNav.length > 0 ? 1 : 0)
          }, minmax(0, 1fr))`,
        }}
      ...
      >
        {primaryMobileNav.map((item) => {
          ...
          return (
            ...
            >
              {active && (
                <span className="absolute top-0 h-[3px] w-6 rounded-b-full bg-teal-400" />
              )}
              {unreadCount > 0 && (
                <span className="absolute right-2 top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-rose-500 px-1 text-[7px] font-black text-white">
              ...
              <span
                className={`flex h-8 w-9 items-center justify-center rounded-xl transition-all ${
                  active
                    ? "bg-teal-500 text-white shadow-[0_6px_16px_rgba(59,130,246,0.35)]"
                    : ""
                }`}
              >
                <Icon
        ...
        {moreMobileNav.length > 0 && (
          ...
          >
            ...
            {moreUnreadCount > 0 && (
              <span className="absolute right-2 top-1 flex h-4 min-w-4 items-center justify-center rounded-full bg-rose-500 px-1 text-[7px] font-black text-white">
                {moreUnreadCount > 9 ? "9+" : moreUnreadCount}
              </span>
            )}
            <span
              className={`flex h-8 w-9 items-center justify-center rounded-xl transition-all ${
            ...
            <div className="grid grid-cols-3 gap-2 p-1">
              {moreMobileNav.map((item) => {
                ...
                return (
                  ...
                  >
                    {unreadCount > 0 && (
                      <span className="absolute right-1.5 top-1.5 flex h-4 min-w-4 items-center justify-center rounded-full bg-rose-500 px-1 text-[7px] font-black text-white">
                        {unreadCount > 9 ? "9+" : unreadCount}
                      </span>
                    )}
                    <span
