...
function route(file,{record={id:1,archive_drive_file_id:'test'},rpc=true,rpcError=null,downloadError=false}={}){const calls=[];const query={select(){return this},eq(key,value){calls.push({key,value});return this},async maybeSingle(){return {data:record,error:null}}};const client={from(){return query},rpc:async()=>({data:rpc,error:rpcError})};return {calls,...load(file,name=>name==='next/server'?{NextResponse}:name==='@supabase/supabase-js'?{createClient:()=>client}:name==='@/lib/publicReferenceCode'?codeLib:name==='@/lib/publicReferenceAccess'?access:name==='@/lib/googleSheetReferences'?{getGoogleSheetReferences:async()=>[{ref_number:'NM/1995/25',subject:'Heshiis',issue_date:'2025-01-01'}]}:name==='@/lib/archiveDriveConfig'?{getArchiveDriveConfig:async()=>({})}:name==='@/lib/driveArchive'?{downloadArchivePdf:async()=>{if(downloadError)throw Error('secret');return Buffer.from('%PDF-1.7')}}:{})};}
...
test('normalizes and verifies historical sheet references without exposing a PDF',async()=>{
 ...
 assert.equal(body.reference.ref_number,'NM/1995/25');assert.equal(body.reference.source,'sheet');assert.equal(body.reference.archive_drive_file_id,null);assert.equal(r.calls.length,0);
});
